...
{
    public sealed class ChooseAnswerExerciseWidget : ExerciseWidget, IExercisePresenter<ChooseAnswerExerciseData>
    {
        [field: SerializeField] public ScrollRect ChooseExerciseScrollView { get; private set; }
        [field: SerializeField] public Transform FooterContainer { get; private set; }
        
        [Header("Prefabs")]
        [SerializeField] private ChooseAnswerExerciseQuestionWidget chooseAnswerQuestionWidgetPrefab;
        [SerializeField] private ExerciseInProgressFooterWidget exerciseFooterWidgetPrefab;
        
        public ImmutableList<ChooseAnswerExerciseQuestionWidget> Questions { get; private set; }

        private ImmutableList<string> _contextIds;
        private ExerciseInProgressFooterWidget _footer;
        private int _currentQuestionIndex;
        
        public void CreateView(ChooseAnswerExerciseData data)
        {
            OpenExerciseWidget();
            
            Questions = ImmutableList<ChooseAnswerExerciseQuestionWidget>.Empty;
        ...
        private void CreateQuestions(ChooseAnswerExerciseData exerciseData)
        {
            var questionsAnswers = exerciseData.QuestionsAnswers;

            for (var i = 0; i < questionsAnswers.Length; i++)
            {
                var questionWidget = Instantiate(chooseAnswerQuestionWidgetPrefab, ChooseExerciseScrollView.content);
        ...
        private void OnNextQuestion()
        {
            ...
            if (!Questions[_currentQuestionIndex].HasAnswer)
            {
                Questions[_currentQuestionIndex].NoAnswerTrigger.Trigger();
                return;
            }
        ...
        private void OpenExerciseWidget()
        {
            ...
            exerciseViewSandbox.FlexBoxWidget.SetValueSmoothly(0.4f);
            exerciseViewSandbox.CurrentExerciseViewSize = 0.4f;
            exerciseViewSandbox.ChooseAnswerExerciseWidget.Show();
        }
        
        private void ShowPoI(ChooseAnswerExerciseData data)
        {
